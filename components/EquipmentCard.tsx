...
export function EquipmentCard({ equipo }: EquipmentCardProps) {
  return (
    <Link href={`/catalogo/${equipo.idveh}`}>
      <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 group">
        ...
        <div className="p-4">
          ...
          <p className="text-sm text-gray-600 mb-3">Código: {equipo.plaveh}</p>
          ...
          <div className="grid grid-cols-2 gap-2 mb-4 text-sm text-gray-600">
            {equipo.potencia && (
              <div className="flex items-center space-x-1">
                ...
                <span>{equipo.potencia} HP</span>
              </div>
            )}
            {equipo.capacidad && (
              <div className="flex items-center space-x-1">
                <Weight className="w-4 h-4 text-primary" />
                <span>{equipo.capacidad}</span>
              </div>
            )}
          </div>
